...
namespace BookRating.Controllers
{
    ...
    public class FeaturedController : ControllerBase
    {
        ...
        {
           return (book.RateAvg*book.RateAvg + m*C) / (book.RateAvg+C);
        }
        [HttpGet]
        public async Task<ActionResult<IEnumerable<Book>>> GetFeaturedBooks([FromQuery] TimeFrame timeFrame, [FromQuery] Category category, int topCount = 10)
        {
            ...
            if (category != null)
            {
                featuredBooksQuery = featuredBooksQuery.Where(b => b.CategoryId == category.Id);
            }
